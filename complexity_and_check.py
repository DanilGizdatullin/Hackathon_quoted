...
def check_quote(quote, min_length=40, max_length=140, one_sentence=True, banned_symbols='[]@<>=', no_nnps=True):
...
def sentence_complexity(sentence, frequency_dist=fdist):
    ...
    for i in word_tokenized:
        if not('\'' in i) and not(',' in i) and not('.' in i) and not('?' in i) \
                and not('!' in i) and not(':' in i) and not('-' in i)\
                and not('"' in i) and not('(' in i) and not(')' in i)\
                and not(i.isdigit()) and not('—' in i) and not('``' in i):
    ...
    word_lemmatized = list(set(word_lemmatized))
...
if __name__ == "__main__":
    ...
    for j in xrange(5):
        print sentence[j]
        if check_quote(sentence[j]):
            print(sentence_complexity(sentence[j], fdist))
